...
import compiler.adapters.GalsParserAdapter;
import compiler.builders.CompilationEngineBuilder;
import compiler.generators.BipCodeGenerator;
...
import compiler.models.CompilationWarning;
import gals.Semantico;
import gals.Sintatico;
import ide.components.*;
import ide.components.MenuBar;
...
public class CompilerIDE extends JFrame {
    ...
    private static final String TITLE = "Integrated Development Environment";
    ...
    private IntermediateCodePanel asmPanel;
    ...
    public CompilerIDE() {
        GalsParserAdapter galsParserAdapter = new GalsParserAdapter(new Sintatico(), new Semantico());
        IIntermediateCodeGenerator intermediateCodeGenerator = new BipCodeGenerator();
        compilationEngine = CompilationEngineBuilder
                .Empty()
                .lexicalAnalyzer(galsParserAdapter)
                .syntaxAnalyzer(galsParserAdapter)
                .semanticAnalyzer(galsParserAdapter)
    ...
    private void initComponents() {
        ...
        asmPanel = new IntermediateCodePanel();
    ...
    private void compile() {
        ...
        try {
            ...
            for (CompilationWarning warning : result.getWarnings()) {
                consolePanel.appendWarning(warning.toString());
            }

            // Update symbol table panel
            symbolTablePanel.update((Graphics) result.getSymbolTable());
            asmPanel.update(result.getIntermediateCode());
            if (result.isSuccess() && result.getIntermediateCode() != null && !result.getIntermediateCode().isBlank()) {
